...
async def approval_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "approve":
        ...
        if not pending:
            await q.edit_message_text("تمت معالجة هذا الطلب مسبقاً.")
            return

        await q.edit_message_text(f"جاري ربط حساب MT5 لـ {pending['full_name']}...")
        ...
        if result["success"]:
            ...
            try:
                await ctx.bot.send_message(
                    user_id,
                    f"✅ تمت الموافقة على طلبك، لكن حدث خطأ في ربط MT5:\n{err}\n\nتواصل مع الدعم."
                )
            except Exception:
                pass
            await q.edit_message_text(f"تمت الموافقة على {pending['full_name']} لكن فشل الربط: {err}")
    else:
        await reject_user(user_id)
        try:
            await ctx.bot.send_message(user_id, "❌ تم رفض طلب التسجيل. تواصل مع الدعم.")
        except Exception:
            pass
...
def _next_lot_prompt(ctx) -> str:
    ...
    return (
        f"القسم: {tier_info['name']} ({TIER_DESCRIPTIONS[tier_num]})\n\n"
        f"أرسل حجم الـ Lot:\n(الافتراضي: {tier_info['default_lot']})"
    )
...
async def got_tp(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    sl_txt     = str(d["sl"]) if d["sl"] else "—"
    tp_txt     = str(d["tp"]) if d["tp"] else "—"
    price_txt  = str(d.get("open_price")) if d.get("open_price") else "—"
    lots_lines = "\n".join([
...
async def cmd_modify(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for t in trades:
        ...
        text += f"ID: {t['id']} | {t['symbol']} {ACTION_LABELS.get(t['action'], t['action'])} | {tier_lbl}\n"
...
async def cmd_close(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    trades = await get_open_trades()
    if not trades:
        await update.message.reply_text("لا توجد صفقات مفتوحة."); return
    kb = []
    for t in trades:
        tier_lbl = get_tier_info(t["target_tier"])["name"] if t["target_tier"] else "الكل"
        kb.append([InlineKeyboardButton(
...
async def close_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(f"تم إغلاق الصفقة #{trade_id}\n\n✅ نجح: {ok} | ❌ فشل: {fail}")
...
async def cmd_kick(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ تم إيقاف العميل {target_id}")
    try:
        await ctx.bot.send_message(target_id, "تم إيقاف حسابك. تواصل مع الدعم.")
    except Exception:
        pass
